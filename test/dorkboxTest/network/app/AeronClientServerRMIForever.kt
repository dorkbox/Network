 ...
 */
class AeronClientServerRMIForever {
    companion object {
        init {
            try {
...
//            configuration.uniqueAeronDirectory = true
        ...
        client.onInit {
            ...
            rmi.save(GarbageObj(123), 456)
        }

        client.onConnect {
            logger.error("connected")

            val obj = this.rmi.get<GarbageObjInt>(123)
            val casted = RemoteObject.cast(obj)
            casted.async = true
            obj.send()
        }
...
//            configuration.enableIPv4 = false
        ...
        configuration.serialization.rmi.register(GarbageObjInt::class.java, GarbageObj::class.java)
...
interface GarbageObjInt {
    fun send()
}
class GarbageObj(val otherId: Int): GarbageObjInt {
    companion object {
        val count = atomic(0L)
    ...
    fun send(connection: Connection) {
        ...
        val obj = connection.rmi.get<GarbageObjInt>(otherId)
        val casted = RemoteObject.cast(obj)
        casted.async = true
        obj.send()
    }
